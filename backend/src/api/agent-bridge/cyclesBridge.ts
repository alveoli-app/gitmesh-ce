...
export const listCycles = async (req, res) => {
    ...
    try {
        ...
        return res.json({
            success: true,
            data: cycles.map((c: any) => c.get({ plain: true })),
        })
    } catch (error: any) {
 ...
 */
export const getCycleMetrics = async (req, res) => {
    const startTime = Date.now()
    ...
    try {
        ...
        const byStatus = {
            backlog: issues.filter((i: any) => i.status === 'backlog').length,
        ...
        const totalHours = issues.reduce((sum: number, i: any) => sum + (i.estimatedHours || 0), 0)
        const completedHours = issues
            .filter((i: any) => i.status === 'done')
            .reduce((sum: number, i: any) => sum + (i.estimatedHours || 0), 0)
