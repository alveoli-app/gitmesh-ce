...
const widgetRepository_1 = __importDefault(require("../repositories/widgetRepository"));
...
async function createSingleTenant() {
    ...
    const as = new activityService_1.default(mockIRepositoryOptions);
    const ts = new tagService_1.default(mockIRepositoryOptions);
    const ms = new memberService_1.default(mockIRepositoryOptions);
    ...
    for (const report of reports) {
        ...
        for (const widget of report.widgets) {
            widgetsArray.push(await widgetRepository_1.default.create(widget, mockIRepositoryOptions));
        }
        report.widgets = widgetsArray.map((i) => i.id);
        // create report with widgets
        await reportRepository_1.default.create(report, mockIRepositoryOptions);
    }
    // create widgets that don't have report relationship
    const widgets = require('./widgets.json');
    for (const widget of widgets) {
        await widgetRepository_1.default.create(widget, mockIRepositoryOptions);
    }
    ...
    log.info(`# of members added: ${await mockIRepositoryOptions.database.member.count({
        tenantId: mockIRepositoryOptions.currentTenant.id,
    })}`);
    log.info(`# of activities added:${await mockIRepositoryOptions.database.activity.count({
        tenantId: mockIRepositoryOptions.currentTenant.id,
    })}`);
    log.info(`# of tags added:${await mockIRepositoryOptions.database.tag.count({
        tenantId: mockIRepositoryOptions.currentTenant.id,
    })}`);
    log.info(`# of reports added:${await mockIRepositoryOptions.database.report.count({
        tenantId: mockIRepositoryOptions.currentTenant.id,
    })}`);
    log.info(`# of widgets added:${await mockIRepositoryOptions.database.widget.count({
        tenantId: mockIRepositoryOptions.currentTenant.id,
    })}`);
}
createSingleTenant();
