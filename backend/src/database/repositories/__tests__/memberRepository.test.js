            ...
            if (usernameData.length > 0) {
                for (const entry of usernameData) {
                    if (typeof entry === 'string') {
                        ...
                        username[platform].push(entry.username);
                    }
                    else {
...
describe('MemberRepository tests', () => {
    ...
    describe('create method', () => {
        it('Should create the given member succesfully', async () => {
            ...
            const cloned = lodash_1.default.cloneDeep(member2add);
            const memberCreated = await memberRepository_1.default.create(cloned, mockIRepositoryOptions);
        ...
        it('Should create succesfully but return without relations when doPopulateRelations=false', async () => {
            ...
            const cloned = lodash_1.default.cloneDeep(member2add);
            const memberCreated = await memberRepository_1.default.create(cloned, mockIRepositoryOptions, false);
            // Trim the hour part from timestamp so we can atleast test if the day is correct for createdAt and joinedAt
            memberCreated.createdAt = memberCreated.createdAt.toISOString().split('T')[0];
            memberCreated.updatedAt = memberCreated.updatedAt.toISOString().split('T')[0];
            const expectedMemberCreated = {
        ...
        it('Should throw error when no username given', async () => {
            ...
            await expect(() => memberRepository_1.default.create(member2add, mockIRepositoryOptions)).rejects.toThrow();
        });
        ...
        it('Should succesfully create member with notes', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const notes1 = await noteRepository_1.default.create({
                body: 'note1',
            }, mockIRepositoryOptions);
            const notes2 = await noteRepository_1.default.create({
                body: 'note2',
            }, mockIRepositoryOptions);
            const member2add = {
                username: {
            ...
            const memberCreated = await memberRepository_1.default.create(member2add, mockIRepositoryOptions);
        ...
        it('Should succesfully create member with organization affiliations', async () => {
            ...
            expect(memberCreated.affiliations.filter((a) => a.segmentId === segment1.id)[0].organizationId).toEqual(org1.id);
            expect(memberCreated.affiliations.filter((a) => a.segmentId === segment2.id)[0].organizationId).toBeNull();
        });
    });
    describe('findById method', () => {
        it('Should successfully find created member by id', async () => {
            ...
            const memberById = await memberRepository_1.default.findById(memberCreated.id, mockIRepositoryOptions);
            // Trim the hour part from timestamp so we can atleast test if the day is correct for createdAt and joinedAt
        ...
        it('Should return a plain object when called with doPopulateRelations false', async () => {
            ...
            const memberById = await memberRepository_1.default.findById(memberCreated.id, mockIRepositoryOptions, true, false);
            // Trim the hour part from timestamp so we can atleast test if the day is correct for createdAt and joinedAt
            memberById.createdAt = memberById.createdAt.toISOString().split('T')[0];
            memberById.updatedAt = memberById.updatedAt.toISOString().split('T')[0];
            expect(memberById).toStrictEqual(expectedMemberFound);
        ...
        it('Should throw 404 error when no member found with given id', async () => {
            ...
            await expect(() => memberRepository_1.default.findById(randomUUID(), mockIRepositoryOptions)).rejects.toThrowError(new common_1.Error404());
        });
    });
    describe('filterIdsInTenant method', () => {
        it('Should return the given ids of previously created member entities', async () => {
            ...
            const member1Returned = await memberRepository_1.default.create(member1, mockIRepositoryOptions);
            const member2Returned = await memberRepository_1.default.create(member2, mockIRepositoryOptions);
            const filterIdsReturned = await memberRepository_1.default.filterIdsInTenant([member1Returned.id, member2Returned.id], mockIRepositoryOptions);
        ...
        it('Should only return the ids of previously created members and filter random uuids out', async () => {
            ...
            const member1Returned = await memberRepository_1.default.create(member1, mockIRepositoryOptions);
            const { randomUUID } = require('crypto');
            const filterIdsReturned = await memberRepository_1.default.filterIdsInTenant([member1Returned.id, randomUUID(), randomUUID()], mockIRepositoryOptions);
        ...
        it('Should return an empty array for an irrelevant tenant', async () => {
            ...
            const filterIdsReturned = await memberRepository_1.default.filterIdsInTenant([member1Returned.id], mockIRepositoryOptions);
    ...
    describe('memberExists method', () => {
        it('Should return the created member for a simple query', async () => {
            ...
            const member1Returned = await memberRepository_1.default.create(member1, mockIRepositoryOptions);
            const found = await memberRepository_1.default.memberExists('test1', types_1.PlatformType.TWITTER, mockIRepositoryOptions);
        ...
        it('Should a plain object when called with doPopulateRelations false', async () => {
            ...
            const found = await memberRepository_1.default.memberExists('test1', types_1.PlatformType.TWITTER, mockIRepositoryOptions, false);
            expect(found).toStrictEqual(member1Returned);
        });
        it('Should return null when non-existent at platform level', async () => {
            ...
            await expect(() => memberRepository_1.default.memberExists('test1', types_1.PlatformType.GITHUB, mockIRepositoryOptions));
        });
        it('Should return null when non-existent at username level', async () => {
            ...
            await memberRepository_1.default.create(member1, mockIRepositoryOptions);
            const memberExists = await memberRepository_1.default.memberExists('test2', types_1.PlatformType.TWITTER, mockIRepositoryOptions);
    ...
    describe('findAndCountAll method', () => {
        it('is successfully finding and counting all members, sortedBy activitiesCount DESC', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const member1 = await memberRepository_1.default.create({
                username: {
                    [types_1.PlatformType.SLACK]: {
                        username: 'test1',
                        integrationId: (0, common_2.generateUUIDv1)(),
            ...
            const member2 = await memberRepository_1.default.create({
                username: {
                    [types_1.PlatformType.SLACK]: {
                        username: 'test2',
                        integrationId: (0, common_2.generateUUIDv1)(),
            ...
            const member3 = await memberRepository_1.default.create({
                username: {
                    [types_1.PlatformType.SLACK]: {
                        username: 'test3',
                        integrationId: (0, common_2.generateUUIDv1)(),
            ...
            await mockIRepositoryOptions.database.activity.bulkCreate([
                {
                    ...
                    memberId: member1.id,
                    username: member1.username[types_1.PlatformType.SLACK],
                    sourceId: '#sourceId1',
                },
                {
                    type: 'message',
                    ...
                    segmentId: mockIRepositoryOptions.currentSegments[0].id,
                    memberId: member2.id,
                    username: member2.username[types_1.PlatformType.SLACK],
                    sourceId: '#sourceId2',
                },
                {
                    type: 'message',
                ...
                },
                {
                    ...
                    segmentId: mockIRepositoryOptions.currentSegments[0].id,
                    memberId: member3.id,
                    username: member3.username[types_1.PlatformType.SLACK],
                    sourceId: '#sourceId4',
                },
                {
                    type: 'message',
            ...
            const members = await memberRepository_1.default.findAndCountAll({ filter: {}, orderBy: 'activityCount_DESC' }, mockIRepositoryOptions);
        ...
        it('is successfully finding and counting all members, sortedBy numberOfOpenSourceContributions DESC', async () => {
            ...
            await memberRepository_1.default.create({
                username: { [types_1.PlatformType.TWITTER]: 'test1' },
                displayName: 'Member 1',
                score: '1',
                joinedAt: new Date(),
                contributions: [
            ...
            await memberRepository_1.default.create({
                username: { [types_1.PlatformType.TWITTER]: 'test2' },
                displayName: 'Member 2',
                score: '6',
                joinedAt: new Date(),
                contributions: [
            ...
            await memberRepository_1.default.create({
                username: { [types_1.PlatformType.TWITTER]: 'test3' },
                displayName: 'Member 3',
                score: '7',
                joinedAt: new Date(),
            }, mockIRepositoryOptions);
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            const members = await memberRepository_1.default.findAndCountAll({ filter: {}, orderBy: 'numberOfOpenSourceContributions_DESC' }, mockIRepositoryOptions);
            expect(members.rows.length).toEqual(3);
        ...
        it('is successfully finding and counting all members, numberOfOpenSourceContributions range gte than 3 and less or equal to 6', async () => {
            ...
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            const members = await memberRepository_1.default.findAndCountAll({ filter: { numberOfOpenSourceContributionsRange: [3, 6] } }, mockIRepositoryOptions);
        ...
        it('is successfully finding and counting all members, and tags [nodejs, vuejs]', async () => {
            ...
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            const members = await memberRepository_1.default.findAndCountAll({ filter: { tags: [nodeTag.id, vueTag.id] } }, mockIRepositoryOptions);
            const member2 = members.rows.find((m) => m.username[types_1.PlatformType.TWITTER][0] === 'test2');
            expect(members.rows.length).toEqual(1);
        ...
        it('is successfully finding and counting all members, and tags [nodejs]', async () => {
            ...
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            const members = await memberRepository_1.default.findAndCountAll({ filter: { tags: [nodeTag.id] } }, mockIRepositoryOptions);
            const member1 = members.rows.find((m) => m.username[types_1.PlatformType.GITHUB][0] === 'test1');
            const member2 = members.rows.find((m) => m.username[types_1.PlatformType.GITHUB][0] === 'test2');
            expect(members.rows.length).toEqual(2);
        ...
        it('is successfully finding and counting all members, and organisations [gitmesh.dev, pied piper]', async () => {
            ...
            await organizationRepository_1.default.addIdentity(gitmeshOrg.id, {
                name: 'gitmesh.dev',
            ...
            await organizationRepository_1.default.addIdentity(pp.id, {
                name: 'pied piper',
            ...
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            const members = await memberRepository_1.default.findAndCountAll({ filter: { organizations: [gitmeshOrg.id, pp.id] } }, mockIRepositoryOptions);
            const member2 = members.rows.find((m) => m.username[types_1.PlatformType.SLACK][0] === 'test2');
            expect(members.rows.length).toEqual(1);
        ...
        it('is successfully finding and counting all members, and scoreRange is gte than 1 and less or equal to 6', async () => {
            ...
            await memberRepository_1.default.create(user1, mockIRepositoryOptions);
            await memberRepository_1.default.create(user2, mockIRepositoryOptions);
            await memberRepository_1.default.create(user3, mockIRepositoryOptions);
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            const members = await memberRepository_1.default.findAndCountAll({ filter: { scoreRange: [1, 6] } }, mockIRepositoryOptions);
            expect(members.rows.length).toEqual(2);
            expect(members.rows.find((m) => m.username[types_1.PlatformType.SLACK][0] === 'test1').score).toEqual(1);
            expect(members.rows.find((m) => m.username[types_1.PlatformType.SLACK][0] === 'test2').score).toEqual(6);
        });
        it('is successfully finding and counting all members, and scoreRange is gte than 7', async () => {
            ...
            await memberRepository_1.default.create(user3, mockIRepositoryOptions);
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            const members = await memberRepository_1.default.findAndCountAll({ filter: { scoreRange: [7] } }, mockIRepositoryOptions);
            expect(members.rows.length).toEqual(1);
        ...
        it('is successfully find and counting members with various filters, computed attributes, and full options (filter, limit, offset and orderBy)', async () => {
            ...
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            let members = await memberRepository_1.default.findAndCountAll({
                filter: {},
                limit: 15,
                offset: 0,
            ...
            members = await memberRepository_1.default.findAndCountAll({
                filter: {
                    reachRange: [55],
                },
                limit: 15,
    ...
    describe('findAndCountAllv2 method', () => {
        it('is successfully finding and counting all members, sortedBy activitiesCount DESC', async () => {
            ...
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            const members = await memberRepository_1.default.findAndCountAllv2({ filter: {}, orderBy: 'activityCount_DESC' }, mockIRepositoryOptions);
            expect(members.rows.length).toEqual(3);
            expect(members.rows[0].activityCount).toEqual('3');
            expect(members.rows[1].activityCount).toEqual('2');
            expect(members.rows[2].activityCount).toEqual('1');
        ...
        it('is successfully finding and counting all members, sortedBy numberOfOpenSourceContributions DESC', async () => {
            ...
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            const members = await memberRepository_1.default.findAndCountAllv2({ filter: {}, orderBy: 'numberOfOpenSourceContributions_DESC' }, mockIRepositoryOptions);
            expect(members.rows.length).toEqual(3);
            expect(members.rows[0].numberOfOpenSourceContributions).toEqual(3);
            expect(members.rows[1].numberOfOpenSourceContributions).toEqual(2);
            expect(members.rows[2].numberOfOpenSourceContributions).toEqual(0);
        ...
        it('is successfully finding and counting all members, numberOfOpenSourceContributions range gte 3', async () => {
            ...
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            const members = await memberRepository_1.default.findAndCountAllv2({
                filter: {
                    and: [
                        {
                            and: [
        ...
        it('is successfully finding and counting all members, and tags [nodejs, vuejs]', async () => {
            ...
            const member2 = members.rows.find((m) => m.username[types_1.PlatformType.TWITTER].includes('test2'));
            expect(members.rows.length).toEqual(1);
        ...
        it('is successfully finding and counting all members, and tags [nodejs]', async () => {
            ...
            const member1 = members.rows.find((m) => m.username[types_1.PlatformType.GITHUB].includes('test1'));
            const member2 = members.rows.find((m) => m.username[types_1.PlatformType.GITHUB].includes('test2'));
            expect(members.rows.length).toEqual(2);
            expect(member1.tags[0].name).toEqual('nodejs');
            expect(member2.tags.map((t) => t.name)).toEqual(expect.arrayContaining(['nodejs', 'vuejs']));
        });
        it('is successfully finding and counting all members, and organisations [gitmesh.dev, pied piper]', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const gitmeshOrg = await organizationRepository_1.default.create({
                identities: [
                    {
                        name: 'gitmesh.dev',
                        url: 'https://gitmesh.dev',
            ...
            const pp = await organizationRepository_1.default.create({
                identities: [
                    {
                        name: 'pied piper',
                        url: 'https://piedpiper.com',
            ...
            const member2 = members.rows.find((m) => m.username[types_1.PlatformType.SLACK].includes('test2'));
            expect(members.rows.length).toEqual(1);
            expect(member2.organizations.map((o) => o.displayName)).toEqual(expect.arrayContaining(['gitmesh.dev', 'pied piper']));
        });
        it('is successfully finding and counting all members, and scoreRange is gte than 7', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const user1 = {
        ...
        it('is successfully find and counting members with various filters, computed attributes, and full options (filter, limit, offset and orderBy)', async () => {
            ...
            await sequelizeTestUtils_1.default.refreshMaterializedViews(db);
            let members = await memberRepository_1.default.findAndCountAllv2({
                filter: {},
                limit: 15,
                offset: 0,
                orderBy: 'activityCount_DESC',
            ...
            expect(members.rows[0].tags[0].name).toEqual('vuejs');
            // filter and order by reach
            members = await memberRepository_1.default.findAndCountAllv2({
                filter: {
                    and: [
                        {
                            and: [
    ...
    describe('update method', () => {
        it('Should succesfully update previously created member', async () => {
            ...
            let cloned = lodash_1.default.cloneDeep(member1);
            const returnedMember = await memberRepository_1.default.create(cloned, mockIRepositoryOptions);
            const updateFields = {
                username: {
                    [types_1.PlatformType.GITHUB]: {
                        username: 'anil_github',
            ...
            cloned = lodash_1.default.cloneDeep(updateFields);
            const updatedMember = await memberRepository_1.default.update(returnedMember.id, cloned, mockIRepositoryOptions);
            ...
            const expectedMemberCreated = {
                ...
                username: mapUsername(Object.assign(Object.assign({}, updateFields.username), member1.username)),
        ...
        it('Should update successfuly but return without relations when doPopulateRelations=false', async () => {
            ...
            const returnedMember = await memberRepository_1.default.create(member1, mockIRepositoryOptions);
            const updateFields = {
                username: {
                    [types_1.PlatformType.GITHUB]: {
                        username: 'anil_github',
            ...
            const updatedMember = await memberRepository_1.default.update(returnedMember.id, updateFields, mockIRepositoryOptions, false);
            // check updatedAt field looks ok or not. Should be greater than createdAt
            expect(updatedMember.updatedAt.getTime()).toBeGreaterThan(updatedMember.createdAt.getTime());
            updatedMember.createdAt = updatedMember.createdAt.toISOString().split('T')[0];
            updatedMember.updatedAt = updatedMember.updatedAt.toISOString().split('T')[0];
        ...
        it('Should successfully update member with given tags', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const tag1 = await tagRepository_1.default.create({ name: 'tag1' }, mockIRepositoryOptions);
            const tag2 = await tagRepository_1.default.create({ name: 'tag2' }, mockIRepositoryOptions);
            const tag3 = await tagRepository_1.default.create({ name: 'tag3' }, mockIRepositoryOptions);
            // Create member with tag3
            let member1 = await memberRepository_1.default.create({
                username: {
                    [types_1.PlatformType.DISCORD]: {
                        username: 'test1',
                        integrationId: (0, common_2.generateUUIDv1)(),
            ...
            member1 = await memberRepository_1.default.update(member1.id, { tags: [tag1.id, tag2.id] }, mockIRepositoryOptions);
            ...
            const { members: _tag1Members } = tag1, tag1Plain = __rest(tag1, ["members"]);
            const { members: _tag2Members } = tag2, tag2Plain = __rest(tag2, ["members"]);
            const expectedMemberCreated = {
                id: member1.id,
        ...
        it('Should successfully update member with given organizations', async () => {
            ...
            const org3 = await organizationRepository_1.default.create({
                displayName: 'hooli',
            ...
            member1 = await memberRepository_1.default.update(member1.id, { organizations: [org1.id, org2.id], organizationsReplace: true }, mockIRepositoryOptions);
            member1.createdAt = member1.createdAt.toISOString().split('T')[0];
            member1.updatedAt = member1.updatedAt.toISOString().split('T')[0];
            member1.organizations = member1.organizations.map((i) => sequelizeTestUtils_1.default.objectWithoutKey(i.get({ plain: true }), ['memberOrganizations']));
            ...
            // strip members field from tags created to expect.
            // we won't be returning second level relationships.
            const { memberCount: _tag1Members } = org1, org1Plain = __rest(org1, ["memberCount"]);
            const { memberCount: _tag2Members } = org2, org2Plain = __rest(org2, ["memberCount"]);
            const expectedMemberCreated = {
                id: member1.id,
                username: member1.username,
                displayName: member1.displayName,
                ...
                joinedAt: new Date(member1.joinedAt),
                organizations: [
                    sequelizeTestUtils_1.default.objectWithoutKey(org1Plain, [
                        'lastActive',
                        'identities',
                        'activeOn',
                        'joinedAt',
                    ...
                    sequelizeTestUtils_1.default.objectWithoutKey(org2Plain, [
                        'lastActive',
                        'identities',
                        'activeOn',
                        'joinedAt',
        ...
        it('Should succesfully update member with notes', async () => {
            ...
            const memberCreated = await memberRepository_1.default.create(member2add, mockIRepositoryOptions);
            const memberUpdated = await memberRepository_1.default.update(memberCreated.id, { notes: [notes1.id, notes2.id] }, mockIRepositoryOptions);
        ...
        it('Should succesfully update member with tasks', async () => {
            ...
            const memberUpdated = await memberRepository_1.default.update(memberCreated.id, { tasks: [tasks1.id, task2.id] }, mockIRepositoryOptions);
        ...
        it('Should throw 404 error when trying to update non existent member', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const { randomUUID } = require('crypto');
            await expect(() => memberRepository_1.default.update(randomUUID(), { location: 'test' }, mockIRepositoryOptions)).rejects.toThrowError(new common_1.Error404());
        });
        it('Should throw a sequelize foreign key error when trying to update a member with a non existing tag', async () => {
            ...
            await expect(() => memberRepository_1.default.update(member1.id, { tags: [randomUUID()] }, mockIRepositoryOptions)).rejects.toThrow();
        });
        it('Should succesfully update member organization affiliations', async () => {
            ...
            const memberUpdated = await memberRepository_1.default.update(memberCreated.id, {
                affiliations: [
                    {
                        segmentId: segment2.id,
                        organizationId: org1.id,
            ...
            expect(memberUpdated.affiliations.filter((a) => a.segmentId === segment2.id)[0].organizationId).toEqual(org1.id);
        });
    });
    describe('destroy method', () => {
        it('Should succesfully destroy previously created member', async () => {
            ...
            const returnedMember = await memberRepository_1.default.create(member1, mockIRepositoryOptions);
            await memberRepository_1.default.destroy(returnedMember.id, mockIRepositoryOptions, true);
            // Try selecting it after destroy, should throw 404
            await expect(() => memberRepository_1.default.findById(returnedMember.id, mockIRepositoryOptions)).rejects.toThrowError(new common_1.Error404());
        });
        it('Should throw 404 when trying to destroy a non existent member', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const { randomUUID } = require('crypto');
            await expect(() => memberRepository_1.default.destroy(randomUUID(), mockIRepositoryOptions)).rejects.toThrowError(new common_1.Error404());
        });
    });
    describe('removeToMerge method', () => {
        it('Should remove a member from other members toMerge list', async () => {
            ...
            const memberCreated1 = await memberRepository_1.default.create(member1, mockIRepositoryOptions);
            const memberCreated2 = await memberRepository_1.default.create(member2, mockIRepositoryOptions);
            await memberRepository_1.default.addToMerge([{ members: [memberCreated1.id, memberCreated2.id], similarity: null }], mockIRepositoryOptions);
            await memberRepository_1.default.addToMerge([{ members: [memberCreated2.id, memberCreated1.id], similarity: null }], mockIRepositoryOptions);
            let m1 = await memberRepository_1.default.findById(memberCreated1.id, mockIRepositoryOptions);
            const m2 = await memberRepository_1.default.findById(memberCreated2.id, mockIRepositoryOptions);
            m1 = await memberRepository_1.default.removeToMerge(memberCreated1.id, memberCreated2.id, mockIRepositoryOptions);
    ...
    describe('addNoMerge method', () => {
        it('Should add a member to other members noMerge list', async () => {
            ...
            const memberCreated1 = await memberRepository_1.default.create(member1, mockIRepositoryOptions);
            const memberCreated2 = await memberRepository_1.default.create(member2, mockIRepositoryOptions);
            let memberUpdated1 = await memberRepository_1.default.addNoMerge(memberCreated1.id, memberCreated2.id, mockIRepositoryOptions);
            const memberUpdated2 = await memberRepository_1.default.addNoMerge(memberCreated2.id, memberCreated1.id, mockIRepositoryOptions);
            memberUpdated1 = await memberRepository_1.default.removeToMerge(memberCreated1.id, memberCreated2.id, mockIRepositoryOptions);
    ...
    describe('removeNoMerge method', () => {
        ...
        it('Should remove a member from other members noMerge list', async () => {
            ...
            let memberUpdated1 = await memberRepository_1.default.addNoMerge(memberCreated1.id, memberCreated2.id, mockIRepositoryOptions);
            const memberUpdated2 = await memberRepository_1.default.addNoMerge(memberCreated2.id, memberCreated1.id, mockIRepositoryOptions);
            memberUpdated1 = await memberRepository_1.default.removeNoMerge(memberCreated1.id, memberCreated2.id, mockIRepositoryOptions);
    ...
    describe('work experiences', () => {
        ...
        async function addWorkExperience(memberId, orgId, data = {}) {
            return await memberRepository_1.default.createOrUpdateWorkExperience(Object.assign({ memberId, organizationId: orgId, source: 'test' }, data), options);
        }
...
//# sourceMappingURL=memberRepository.test.js.map