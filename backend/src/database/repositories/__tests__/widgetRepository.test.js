...
describe('WidgetRepository tests', () => {
    ...
    describe('create method', () => {
        it('Should create a widget succesfully with default values', async () => {
            ...
            const widgetCreated = await widgetRepository_1.default.create(widget2Add, mockIRepositoryOptions);
        ...
        it('Should create a widget succesfully with given values -- without report', async () => {
            ...
            const { report: _reportObj } = widgetCreated, widgetWithoutReport = __rest(widgetCreated, ["report"]);
            const widgetExpected = {
                id: widgetCreated.id,
                type: widget2Add.type,
        ...
        it('Should throw sequelize not null error -- type field is required', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const widget2Add = {};
            await expect(() => widgetRepository_1.default.create(widget2Add, mockIRepositoryOptions)).rejects.toThrow();
        });
    });
    describe('findById method', () => {
        it('Should successfully find created widget by id', async () => {
            ...
            const widgetById = await widgetRepository_1.default.findById(widgetCreated.id, mockIRepositoryOptions);
            ...
            const { report: _reportObj } = widgetById, widgetWithoutReport = __rest(widgetById, ["report"]);
            expect(widgetWithoutReport).toStrictEqual(widgetExpected);
        });
        it('Should throw 404 error when no widget found with given id', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const { randomUUID } = require('crypto');
            await expect(() => widgetRepository_1.default.findById(randomUUID(), mockIRepositoryOptions)).rejects.toThrowError(new common_1.Error404());
        });
    });
    describe('findByType method', () => {
        it('Should successfully find one widget by type', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const widgetCreated = await widgetRepository_1.default.create({ type: 'test-widget' }, mockIRepositoryOptions);
            widgetCreated.createdAt = widgetCreated.createdAt.toISOString().split('T')[0];
            widgetCreated.updatedAt = widgetCreated.updatedAt.toISOString().split('T')[0];
            const widgetExpected = {
                id: widgetCreated.id,
            ...
            const widgetByType = await widgetRepository_1.default.findByType(widgetCreated.type, mockIRepositoryOptions);
            ...
            const { report: _reportObj } = widgetByType, widgetWithoutReport = __rest(widgetByType, ["report"]);
            expect(widgetWithoutReport).toStrictEqual(widgetExpected);
        });
        it('Should throw 404 error when no widget found with given type', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            await widgetRepository_1.default.create({ type: 'some-type' }, mockIRepositoryOptions);
            await expect(() => widgetRepository_1.default.findByType('some-other-type', mockIRepositoryOptions)).rejects.toThrowError(new common_1.Error404());
        });
    });
    describe('filterIdsInTenant method', () => {
        it('Should return the given ids of previously created widget entities', async () => {
            ...
            const widget1Created = await widgetRepository_1.default.create(widget1, mockIRepositoryOptions);
            const widget2Created = await widgetRepository_1.default.create(widget2, mockIRepositoryOptions);
            const filterIdsReturned = await widgetRepository_1.default.filterIdsInTenant([widget1Created.id, widget2Created.id], mockIRepositoryOptions);
        ...
        it('Should only return the ids of previously created widgets and filter random uuids out', async () => {
            ...
            const widgetCreated = await widgetRepository_1.default.create(widget, mockIRepositoryOptions);
            const { randomUUID } = require('crypto');
            const filterIdsReturned = await widgetRepository_1.default.filterIdsInTenant([widgetCreated.id, randomUUID(), randomUUID()], mockIRepositoryOptions);
        ...
        it('Should return an empty array for an irrelevant tenant', async () => {
            ...
            const filterIdsReturned = await widgetRepository_1.default.filterIdsInTenant([widgetCreated.id], mockIRepositoryOptions);
    ...
    describe('findAndCountAll method', () => {
        it('Should find and count all widgets, with various filters', async () => {
            ...
            const widget3Created = await widgetRepository_1.default.create(widget3, mockIRepositoryOptions);
            await new Promise((resolve) => {
                setTimeout(resolve, 50);
            });
            const widget4Created = await widgetRepository_1.default.create(widget4, mockIRepositoryOptions);
            // Filter by type
            // Current findAndCountAll uses wildcarded like statement so it matches both widget1 and widget4
            let widgets = await widgetRepository_1.default.findAndCountAll({ filter: { type: 'number-activities' } }, mockIRepositoryOptions);
            expect(widgets.count).toEqual(2);
            expect(widgets.rows).toStrictEqual([widget4Created, widget1Created]);
            // Filter by id
            widgets = await widgetRepository_1.default.findAndCountAll({ filter: { id: widget1Created.id } }, mockIRepositoryOptions);
            ...
            widgets = await widgetRepository_1.default.findAndCountAll({
                filter: {
            ...
            expect(widgets.rows).toStrictEqual([widget1Created]);
            // Filter by title
            widgets = await widgetRepository_1.default.findAndCountAll({ filter: { title: 'graph' } }, mockIRepositoryOptions);
            expect(widgets.count).toEqual(2);
            expect(widgets.rows).toStrictEqual([widget2Created, widget1Created]);
            // Filter by report1
            widgets = await widgetRepository_1.default.findAndCountAll({ filter: { report: report1Created.id } }, mockIRepositoryOptions);
            expect(widgets.count).toEqual(2);
            expect(widgets.rows).toStrictEqual([widget2Created, widget1Created]);
            // Filter by report2
            widgets = await widgetRepository_1.default.findAndCountAll({ filter: { report: report2Created.id } }, mockIRepositoryOptions);
            expect(widgets.count).toEqual(1);
    ...
    describe('update method', () => {
        it('Should succesfully update previously created widget', async () => {
            ...
            const widgetCreated = await widgetRepository_1.default.create(widget1, mockIRepositoryOptions);
            ...
            const widgetUpdated = await widgetRepository_1.default.update(widgetCreated.id, {
            ...
            const { report: _reportObj } = widgetUpdated, widgetWithoutReport = __rest(widgetUpdated, ["report"]);
        ...
        it('Should throw 404 error when trying to update non existent widget', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const { randomUUID } = require('crypto');
            await expect(() => widgetRepository_1.default.update(randomUUID(), { type: 'non-existent' }, mockIRepositoryOptions)).rejects.toThrowError(new common_1.Error404());
        });
    });
    describe('destroy method', () => {
        it('Should succesfully destroy previously created widget', async () => {
            ...
            const returnedWidget = await widgetRepository_1.default.create(widget, mockIRepositoryOptions);
            await widgetRepository_1.default.destroy(returnedWidget.id, mockIRepositoryOptions, true);
            // Try selecting it after destroy, should throw 404
            await expect(() => widgetRepository_1.default.findById(returnedWidget.id, mockIRepositoryOptions)).rejects.toThrowError(new common_1.Error404());
        });
        it('Should throw 404 when trying to destroy a non existent widget', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const { randomUUID } = require('crypto');
            await expect(() => widgetRepository_1.default.destroy(randomUUID(), mockIRepositoryOptions)).rejects.toThrowError(new common_1.Error404());
        });
    });
});
//# sourceMappingURL=widgetRepository.test.js.map