 ...
 */
const issueService_1 = __importDefault(require("../../api/devtel/issues/issueService"));
...
describe('DevtelIssueService tests', () => {
    ...
    describe('Issue CRUD operations', () => {
        it('Should create an issue with required fields', async () => {
            const mockOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            // Mock project context
            const mockProject = { id: 'test-project-id', key: 'TEST' };
            const service = new issueService_1.default(mockOptions, mockProject.id);
            const issueData = {
                title: 'Test Issue',
                type: 'story',
                status: 'backlog',
    ...
    describe('Bulk operations', () => {
        it('Should bulk update issues', async () => {
            ...
            const result = await service.bulkUpdate([issue1.id, issue2.id], { priority: 'high' });
...
//# sourceMappingURL=devtelIssueService.test.js.map