...
describe('TaskService tests', () => {
    ...
    describe('Assign to user by ID', () => {
        it('Should work when sending null or undefined', async () => {
            ...
            const service = new taskService_1.default(mockIRepositoryOptions);
        ...
        it('Should work when sending changing assignee user with another user', async () => {
            const mockIRepositoryOptions = await sequelizeTestUtils_1.default.getTestIRepositoryOptions(db);
            const user2 = await userRepository_1.default.create(await sequelizeTestUtils_1.default.getRandomUser(), mockIRepositoryOptions);
            // add user to tenant
