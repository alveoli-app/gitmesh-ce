...
export default class ChatService extends LoggerBase {
    ...
    async listConversations(filters: {
        ...
        return {
            rows: conversations.rows.map((c: any) => c.get({ plain: true })),
     ...
     */
    async getConversation(conversationId: string, includeMessages: boolean = true) {
        ...
        if (includeMessages) {
            ...
            result.messages = messages.map((m: any) => m.get({ plain: true }))
        }
    ...
    async getMessages(conversationId: string, options: {
        ...
        return messages.map((m: any) => m.get({ plain: true }))
    }
     ...
     */
    async createProposal(data: ActionProposalInput) {
     ...
     */
    async getPendingProposals(conversationId: string) {
        ...
        return proposals.map((p: any) => p.get({ plain: true }))
    }
    ...
    private async processWithAgent(
        ...
        try {
            ...
            const agentToolSets: Record<string, string[]> = {}
            for (const config of agentConfigs) {
                if (config.configuration?.allowedToolSets) {
            ...
            await agentMessage.update({
                ...
                durationMs: Date.now() - startTime,
                metadata: {
                    proposalIds: proposals.map((p: any) => p.id),
                },
            })
     ...
     */
    private async buildContext(conversation: any, userMessage: any) {
        const context: any = {
            conversation: {
                id: conversation.id,
        ...
        if (conversation.projectId) {
            const project = await this.options.database.devtelProjects.findByPk(
            ...
            if (project) {
                context.project = {
                    ...
                    lead: project.lead?.fullName,
                }
            }
        }

        // Parse mentioned entities from user message
        const mentionedEntities = userMessage.metadata?.mentionedEntities || []
        ...
        context.recentMessages = recentMessages.reverse().map((m: any) => ({
     ...
     */
    private async summarizeConversation(conversationId: string) {
        try {
            ...
            if (messages.length < 10) return // Not enough to summarize
     ...
     */
    private async loadMentionedEntities(entities: Array<{ type: string; id: string }>) {
        const loaded: any[] = []
     ...
     */
    private async executeAction(actionType: string, parameters: Record<string, any>) {
        switch (actionType) {
            case 'create_issue':
    ...
    private async executeCreateIssue(params: any) {
    ...
    private async executeUpdateIssue(params: any) {
        ...
        await issue.update({
            ...
            ...(params.estimatedHours !== undefined && { estimatedHours: params.estimatedHours }),
    ...
    private async executeAssignIssue(params: any) {
        const issue = await this.options.database.devtelIssues.findByPk(params.issueId)
        if (!issue) throw new Error('Issue not found')
    ...
    private async executeCreateSpec(params: any) {
     ...
     */
    private isActionReversible(actionType: string): boolean {
     ...
     */
    private async handleAgentError(messageId: string, errorMessage: string) {
        try {
    ...
    private async generateConversationTitle(conversationId: string, firstMessage: string) {
        try {
            ...
            if (response.data?.title) {
                await this.options.database.chatConversations.update(
